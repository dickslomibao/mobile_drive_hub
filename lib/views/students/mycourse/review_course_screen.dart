...
class ReviewCourseScreen extends StatefulWidget {
  const ReviewCourseScreen({super.key, required});

  @override
  State<ReviewCourseScreen> createState() => _ReviewCourseScreenState();
}

class _ReviewCourseScreenState extends State<ReviewCourseScreen> {
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              ...
                              final read =
                                  context.read<ViewMyCoursesController>();

                              String r = await read.createReview(
                                {
                                  'school_id': read.course['info']['d_user_id'],
                                  'course_id': read.course['info']
                                      ['main_course_id'],
                                  'order_list_id': read.course['info']
                                      ['mycourse_id'],
                                  'content': c.text,
                                  'hide': hide ? 2 : 1,
                                  'rating': rating,
                              ...
                              if (context.mounted) {
                                ...
                                if (r == 'success') {
                                  QuickAlert.show(
                                    ...
                                    onConfirmBtnTap: () {
                                      read.getStudentSingleCourses(
                                        int.parse(
                                          read.course['info']['mycourse_id']
                                              .toString(),
                                        ),
                                      );
                                      Navigator.of(context).pop();
